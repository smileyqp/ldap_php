...
if(ldap_errno($ldap_conn)!=0){
    ...
    ldap_delete($ldap_conn,$list_user);


    $info["cn"] = $listname;
    $info["sn"] = $listname;
    $info["userPassword"][0] = "{MD5}".base64_encode(pack("H*",md5($listpwd)));
    $info["objectclass"] = "inetOrgPerson";
    $r = ldap_add($ldap_conn, "cn=".$listname.", ou=groups, dc=smileyqp,dc=com", $info);

    echo '您好'.$_COOKIE["user"].',用户'.$listname.'已经禁用！';
